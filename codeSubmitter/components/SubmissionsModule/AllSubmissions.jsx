...
export default function AllSubmissionsTable() {
  ...
  useEffect(() => {
    const fetchSubmissions = async () => {
      ...
      const response = await fetch(
        "https://tuf-interview.vercel.app/api/submissions",
        options
      );
  ...
  return (
    <div className="w-full flex flex-col min-h-screen items-center relative">
      <Link to="/" className="font-semibold underline absolute top-14 right-16">
        Go to home
      </Link>
            ...
            {submissions?.map((submission, index) => {
              return (
                <tr key={index} className="bg-white border-b">
                  ...
                  <td className="px-6 py-4">{submission.stdin}</td>
                  <td className="px-6 py-4">
                    {submission.code.length > 100 ? (
                      <>
                        <div
                          className={`${
                            showCode[index]?.show || "overflow-hidden h-5"
                          }`}
                        >
                          {submission.code}
                        </div>
                        ...
                        >
                          {showCode[index]?.show ? "Hide" : "Show more"}
                        </button>
                      </>
                    ) : (
                      submission.code
